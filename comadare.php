...
while($row_cart = mysqli_fetch_array($run_cart)){
    ...
    while($row_products = mysqli_fetch_array($run_produse)){
        ...
        $inserare_prod_com="INSERT INTO produse_comandate (order_id,id_produs,nume_produs,qty,size,color,subtotal,order_status) values ('$order_id','$pro_id ','$pro_title','$pro_qty','$pro_size','$pro_color','$sub_total','$status')";
        $run_insertcom=mysqli_query($conn,$inserare_prod_com);
    }}
        $inserare_client="INSERT INTO comenzi (order_id,nume_client,prenume_client,adresa,judet,localitate,cod_postal,order_date,total,order_status,email,telefon) values ('".$order_id."','".$nume."','".$prenume."','".$adresa."','".$judet."','".$localitate."','".$codpostal."','".$date."','".$total."','".$status."','".$email."','".$telefon."')";
